...
fb.api.Query.prototype.awaitQ = function<T>($q: ng.IQService): ng.IPromise<T>
{
    var self: Firebase = this;
    var d = $q.defer<T>();

    var subs = self.on('value', s =>
    {
        var val = s.val();
        if (!val)
        {
...
fb.api.Query.prototype.valueQ = function<T>($q: ng.IQService): ng.IPromise<T>
{
    var self: Firebase = this;
    var d = $q.defer<T>();
